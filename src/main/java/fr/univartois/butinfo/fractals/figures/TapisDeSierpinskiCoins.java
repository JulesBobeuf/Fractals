...
public class TapisDeSierpinskiCoins extends CreerFractaleAvecFigures {
     ...
     * @param nbRep
     */
    public TapisDeSierpinskiCoins(int nbRep) {
        super(900, 900, "images/figures/TapisDeSierpinskiCoins.svg", nbRep);
    }
     ...
     * @param nb
     */
    private void construit(int x, int y, int width, int nb, String color) {
        if (nb == nbRep) {
            return;
        } else {
            Rectangle r1 = new Rectangle(x + width, y + width, width, width, color);
            composition.add(r1);

            construit(x, y, width / 3, nb + 1, "black");
            construit(x + width, y, width / 3, nb + 1, "white");
            construit(x + 2 * (width), y, width / 3, nb + 1, "black");
            construit(x, y + width, width / 3, nb + 1, "white");
            construit(x + 2 * (width), y + width, width / 3, nb + 1, "white");
            construit(x, y + 2 * (width), width / 3, nb + 1, "black");
            construit(x + width, y + 2 * (width), width / 3, nb + 1, "white");
            construit(x + 2 * (width), y + 2 * (width), width / 3, nb + 1, "black");
